...
--- @class weave.view.PrefsState
--- @field show_thoughts boolean
--- @field show_diffs boolean
--- @field conceal_markdown boolean
--- @field follow boolean

--- @class weave.view.Prefs
--- @field state weave.view.PrefsState
--- @field _subscribers fun(state: weave.view.PrefsState)[]
local Prefs = {}
Prefs.__index = Prefs

--- @return weave.view.Prefs
function Prefs:new()
  return setmetatable({
    state = {
      show_thoughts = true,
...
--- @param fn fun(state: weave.view.PrefsState)
...
function Prefs:set(key, value)
  if self.state[key] == nil then
    error("weave: unknown view pref '" .. tostring(key) .. "'")
  end
  local draft = {}
