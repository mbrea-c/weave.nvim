...
--- `vim.b[bufnr].clanker_slash_commands` by the on_create wiring below.
...
function M.slash_complete(findstart, _base)
  ...
  return vim.b[vim.api.nvim_get_current_buf()].clanker_slash_commands or {}
end
...
local function wire_completion(store, bufnr)
  vim.b[bufnr].clanker_slash_commands = store:get_commands()
  local unsubscribe
  unsubscribe = store:subscribe(function(state)
    ...
    vim.b[bufnr].clanker_slash_commands = state.commands
  end)

  vim.bo[bufnr].completeopt = "menu,menuone,noinsert,popup,fuzzy"
  vim.bo[bufnr].iskeyword = vim.bo[bufnr].iskeyword .. ",-"
  vim.bo[bufnr].completefunc = "v:lua.require'clanker.view.prompt'.slash_complete"

  vim.api.nvim_create_autocmd("TextChangedI", {
    buffer = bufnr,
    callback = function()
      local commands = vim.b[bufnr].clanker_slash_commands or {}
      if #commands == 0 then
        return
      end
...
--- @param ctx table
--- @param props { store: clanker.store.SessionStore, on_submit: fun(text: string), on_steer: fun(text: string), height?: integer, on_create?: fun(bufnr: integer) }
      ...
      on_create = function(bufnr)
        ...
        for _, mode in ipairs({ "n", "i" }) do
          vim.keymap.set(mode, "<C-x>", function()
            ...
            props.on_steer(text)
          end, { buffer = bufnr, desc = "clanker: steer (interrupt + send)" })
        end
