...
--- @param entry clanker.registry.Entry
--- @return string
...
--- @class clanker.view.SessionModalHandle
--- @field bufnr integer
--- @field winid integer
--- @field close fun()
...
--- @param opts { registry?: table, on_select: fun(entry: clanker.registry.Entry), on_new?: fun(), on_load_saved?: fun() }
---   on_select/on_new/on_load_saved run AFTER the modal closes itself.
--- @return clanker.view.SessionModalHandle handle
function M.open(opts)
  local registry = opts.registry or require("clanker.registry")
  ...
  for _, lhs in ipairs({ "q", "<Esc>" }) do
    vim.keymap.set("n", lhs, close, { buffer = app.bufnr, nowait = true, desc = "clanker: close session modal" })
  end
  app.focus()
