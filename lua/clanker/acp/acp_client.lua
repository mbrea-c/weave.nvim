...
local transport_module = require("clanker.acp.acp_transport")
...
--- @class clanker.acp.ACPClientData
--- @field provider_config clanker.acp.ACPProviderConfig
--- @field id_counter number
--- @field state clanker.acp.ClientConnectionState
--- @field protocol_version number
--- @field client_info clanker.acp.ClientInfo
--- @field capabilities clanker.acp.ClientCapabilities
--- @field agent_capabilities? clanker.acp.AgentCapabilities
--- @field agent_info? clanker.acp.AgentInfo
--- @field auth_methods clanker.acp.AuthMethod[]
--- @field callbacks table<number, fun(result: table|nil, err: clanker.acp.ACPError|nil)>
--- @field transport? clanker.acp.ACPTransportInstance
--- @field ready_listeners fun(client: clanker.acp.ACPClient)[]
--- @field subscribers table<string, clanker.acp.ClientHandlers>

--- @class clanker.acp.ACPClient : clanker.acp.ACPClientData
--- @field _on_ready fun(client: clanker.acp.ACPClient)
local ACPClient = {}
...
--- @param config clanker.acp.ACPProviderConfig
--- @param on_ready fun(client: clanker.acp.ACPClient)
--- @return clanker.acp.ACPClient client
function ACPClient:new(config, on_ready)
  --- @type clanker.acp.ACPClientData
  local instance = {
  ...
  local client = setmetatable(instance, self) --[[@as clanker.acp.ACPClient]]
...
--- @param callback fun(client: clanker.acp.ACPClient)
...
--- @param handlers clanker.acp.ClientHandlers
...
--- @protected
--- @param session_id string
--- @param callback fun(sub: clanker.acp.ClientHandlers): nil
...
function ACPClient:_setup_transport()
  ...
  if transport_type == "stdio" then
    --- @type clanker.acp.StdioTransportConfig
    ...
    --- @type clanker.acp.TransportCallbacks
    local callbacks = {
    ...
    self.transport = transport_module.create_stdio_transport(transport_config, callbacks)
  else
    error("Unsupported transport type: " .. transport_type)
  end
end

--- @param state clanker.acp.ClientConnectionState
...
--- @param data any|nil
--- @return clanker.acp.ACPError error
...
--- @param callback fun(result: table|nil, err: clanker.acp.ACPError|nil)
...
--- @param message clanker.acp.ResponseRaw
  ...
  if
    not (
      ...
      and (
        ...
        or message.params.update.sessionUpdate == "agent_thought_chunk"
      )
    )
  then
...
--- @protected
--- @param update clanker.acp.ToolCallBase
--- @return clanker.ui.MessageWriter.ToolCallBlock message
function ACPClient:__build_tool_call_message(update)
  --- @type clanker.ui.MessageWriter.ToolCallBlock
  local message = {
  ...
  -- Carry raw input/output verbatim for ALL kinds so the UI has full tool
  -- detail (args + results), notably for MCP/other tools that send no
    ...
    for _, content in ipairs(update.content) do
      if content then
        if content.type == "content" and content.content and content.content.text then
          table.insert(body_parts, self:safe_split(content.content.text))
...
--- @protected
--- @param session_id string
--- @param update clanker.acp.ToolCallMessage
...
--- @protected
--- @param session_id string
--- @param update clanker.acp.ToolCallUpdate
...
--- @protected
--- @param message_id number
--- @param request clanker.acp.RequestPermission
function ACPClient:__handle_request_permission(message_id, request)
  ...
  self:__with_subscriber(session_id, function(subscriber)
    ...
    subscriber.on_request_permission(request, function(option_id)
      --- A nil option_id means the request was cancelled (e.g. the turn
      --- was steered/cancelled). The ACP prompt-turn spec requires the
      --- client to answer pending permission requests with the
      --- `cancelled` outcome rather than a selection.
      --- @type clanker.acp.RequestPermissionOutcome
      local outcome = option_id == nil and { outcome = "cancelled" } or { outcome = "selected", optionId = option_id }
...
function ACPClient:_connect()
  ...
  if self.state ~= "connected" then
    local error = self:__create_error(self.ERROR_CODES.PROTOCOL_ERROR, "Cannot initialize: client not connected")
    return error
  end

  self:_set_state("initializing")

  --- @type clanker.acp.InitializeParams
  local init_params = {
  ...
  self:_send_request("initialize", init_params, function(result, err)
    if not result or err then
      ...
      Logger.notify("Failed to initialize\n\n" .. vim.inspect(err), vim.log.levels.ERROR)
      return
    end

    --- @cast result clanker.acp.InitializeResponse
...
--- @param handlers clanker.acp.ClientHandlers
--- @param callback fun(result: clanker.acp.SessionCreationResponse|nil, err: clanker.acp.ACPError|nil)
--- @param mcp_servers? clanker.acp.McpServer[] MCP servers for the agent to spawn (session/new mcpServers). Defaults to none.
function ACPClient:create_session(handlers, callback, mcp_servers)
  ...
  self:_send_request("session/new", {
    ...
    if not result then
      err = self:__create_error(self.ERROR_CODES.PROTOCOL_ERROR, "Failed to create session: missing result")
    ...
    --- @cast result clanker.acp.SessionCreationResponse
    callback(result, nil)
  end)
end
...
--- @param handlers clanker.acp.ClientHandlers
--- @param on_load_complete fun(err: clanker.acp.ACPError|nil, result?: clanker.acp.SessionCreationResponse)|nil
function ACPClient:load_session(session_id, cwd, mcp_servers, handlers, on_load_complete)
  if not self.agent_capabilities or not self.agent_capabilities.loadSession then
    ...
    if on_load_complete then
      on_load_complete(self:__create_error(-1, "Agent does not support loading sessions"))
    end
    return
  end
  ...
  self:_send_request("session/load", {
    ...
    if on_load_complete then
      -- `result` is forwarded as a 2nd arg so callers can capture session
      -- config (models/modes) that session/load may return, like
      -- session/new. Positional, so existing function(err) callers are
      -- unaffected.
      on_load_complete(err, result)
    end
  end)
end

--- @param cwd string
--- @param callback fun(result: clanker.acp.SessionListResponse|nil, err: clanker.acp.ACPError|nil)
function ACPClient:list_sessions(cwd, callback)
  local caps = self.agent_capabilities
  if not caps or not caps.sessionCapabilities or not caps.sessionCapabilities.list then
    callback(nil, self:__create_error(self.ERROR_CODES.PROTOCOL_ERROR, "Agent does not support listing sessions"))
    return
  end
  ...
  self:_send_request("session/list", {
    ...
    if type(result) ~= "table" or not result.sessions then
      callback(
        nil,
        self:__create_error(self.ERROR_CODES.PROTOCOL_ERROR, "Malformed session/list response: missing sessions field")
      )
      return
    end

    --- @cast result clanker.acp.SessionListResponse
    callback(result, nil)
  end)
end

--- @param session_id string
--- @param prompt clanker.acp.Content[]
--- @param callback fun(result: table|nil, err: clanker.acp.ACPError|nil)
...
--- @param callback fun(result: table|nil, err: clanker.acp.ACPError|nil)
function ACPClient:set_config_option(session_id, config_id, config_value, callback)
  local params = {
    sessionId = session_id,
    configId = config_id,
    value = config_value,
...
--- Cancel only the IN-FLIGHT TURN, keeping the session and its subscriber alive
--- so a subsequent prompt on the same session still streams to the UI. This is
--- the "steer" / interrupt primitive, distinct from cancel_session (teardown).
---
--- Per the ACP prompt-turn spec, after sending session/cancel the agent stops
--- work and responds to the original session/prompt with the `cancelled` stop
--- reason; the caller resends once that arrives. The caller is responsible for
--- resolving any pending session/request_permission with the cancelled outcome.
--- @param session_id string
function ACPClient:cancel_turn(session_id)
  if not session_id then
    return
  end
