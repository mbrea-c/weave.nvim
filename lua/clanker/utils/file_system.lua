...
local BufHelpers = require("clanker.utils.buf_helpers")

--- @class clanker.utils.FileSystem
local FileSystem = {}
...
function FileSystem.save_to_disk(abs_path, content)
  ...
  return false, "Failed to open file for writing: " .. abs_path .. ". Error: " .. tostring(open_err)
end
...
function FileSystem.read_file(abs_path, line, limit, callback)
  ...
  if err ~= nil then
    Logger.notify("Agent file read error: " .. err, vim.log.levels.ERROR, { title = " Read file error" })
    callback(nil)
    return
  end
