...
# invisibly fast on a local terminal become visible lag. demo/slowpty.py is
# the throttle (real backpressure, bounded lag); needs python3 on PATH.
#   make demo-constrained
#   make demo-constrained DEMO_BPS=2400
DEMO_BPS ?= 9600
.PHONY: demo-constrained
demo-constrained:
	python3 demo/slowpty.py $(DEMO_BPS) $(NVIM_BIN) --clean -u demo/init.lua
