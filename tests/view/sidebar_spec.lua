...
describe("view.sidebar", function()
  it("renders every section for a fresh session", function()
    ...
    assert.truthy(text:find("Preset: Ask  (;;p)", 1, true))
  ...
  it("projects session meta and permission mode live", function()
    ...
    Permissions.set_mode("on") -- cycling walks the presets for the mode in force
    Permissions.cycle() -- the engine, not the store: the preset is editor-global
    assert.truthy(text_of(handle.bufnr):find("Preset: Read-only  (;;p)", 1, true))
    handle.unmount()
    Permissions._reset()
  end)
